...
class GlobalAttention(nn.Module):
    def __init__(
        self, transformer_num_heads=8, hidden_size=512, transformer_dropout_rate=0.0
    ):
        super().__init__()
...
class convBlock(nn.Module):
    def __init__(self, inplace, outplace, kernel_size=3, padding=1):
        ...
        self.conv1 = nn.Conv2d(
            inplace, outplace, kernel_size=kernel_size, padding=padding, bias=False
        )
...
class GlobalLocalBrainAge(nn.Module):
    def __init__(
        self, inplace, patch_size=64, step=-1, nblock=6, drop_rate=0.5, backbone="vgg8"
    ):
        """
        Parameter:
        ...
        if backbone == "vgg8":
            self.global_feat = vnet.VGG8Global(inplace)
            self.local_feat = vnet.VGG8Local(inplace)
            hidden_size = 512
        elif backbone == "vgg16":
    ...
    def forward(self, xinput):
        ...
        for y in range(0, H - self.patch_size, self.step):
            for x in range(0, W - self.patch_size, self.step):
                ...
                featurelist.append(xloc)
                out = self.locout(xloc)
                outlist.append(out)
        # Pre-process the outputs for classification:
        # Remove global pathway and detach the gradients   
        # If the tensor is not in the shape [n,3,169,208], and n is not bigger than 1, 
        # it will return an error because of the dimension squeezes
           
        outlist = outlist[1:]
        for i in range(len(outlist)):
            outlist[i]=outlist[i].detach()
        for i in range(len(featurelist)):
            featurelist[i]=featurelist[i].detach()
        # Squeeze the tensor dimension
        for i in range(len(featurelist)):
            featurelist[i] = featurelist[i].squeeze(0)
              
        # Stack all the patches into a single tensor
        feature_tensor = torch.stack(featurelist)
