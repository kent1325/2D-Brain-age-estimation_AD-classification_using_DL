...
class VGG8(nn.Module):
    ...
    def forward(self,x):
        x = self.conv11(x)
        x = self.conv12(x)
        #print(f'first layer has shape {x.shape}')
        x = self.maxp(x)
        #print(f'first layer has shape {x.shape}')
        ...
        x = self.conv42(x)
        x = self.maxp(x)
        #print(f'last GLOBAL layer has shape {x.shape}')
        
        return x

class VGG8Global(nn.Module):
    def __init__(self,inplace):
        super().__init__()
...
class VGG8Local(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.conv42(x)
        x = self.maxp(x)
        #print(f'last LOCAL layer has shape {x.shape}')
